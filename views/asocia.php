...
<body>
  ...
  <thead>
    <tr class="text-center">
      <th scope="col">ID</th>
      <th scope="col">DNI</th>
      <th scope="col">CÓDIGO DEL PERFIL</th>
  ...
  <tbody>
    ...
    <?php include_once("../php/listar_asocia.php");?>
    <?php foreach ($asocia as $asocia) {?>

    <!-- SE MOSTRARÁ CADA UNO DE LOS REGISTROS" POR COLUMNAS-->
  
    <tr class="text-center">
      <td><?php echo $asocia->id_asociacion;?></td>
      <td><?php echo $asocia->dni;?></td>
      <td><?php echo $asocia->cod_perfil;?></td>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Nueva Asociación</h5>
      ...
      <div class="modal-body">
        ...
        <!-- FORMULARIO -->
        <form action="../php/insertar_asocia.php" method="POST">
          <div class="form-group">
            <label>ID</label>
            <input type="integer" name="id_asociacion" class="form-control">
          </div>
...
<div class="modal fade" id="editar" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Editar Asociación</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <!-- FORMULARIO -->
        <form action="../php/editar_asocia.php" method="POST">
          <input type="hidden" name="id_asociacion" id="update_id">
          ...
          <div class="form-group">
            <label>CÓDIGO DEL PERFIL</label>
            <input type="integer" name="cod_perfil" id="cod_perfil" class="form-control">
          </div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body"> <!-- AQUI SE REALIZARÁ LA BAJA -->
        ...
        <form action="../php/eliminar_asocia.php" method="POST">
        <input type="hidden" name="id_asociacion" id="eliminar_id"> <!-- DE TIPO HIDDEN PARA OCULTAR LOS DATOS --> 
...
<!-- AQUI NOS MOSTRARÁ INFORMACIÓN DE 3 TABLAS JUNTAS, PARA SABER QUE CLIENTE Y PERFIL SE SOLICITAN -->
  <center><h4>Selección para saber el nombre del demandante y descripción del perfil</h4> 
<center><table class="table table-hover" style="width: 50rem;">
    <thead>
        <tr>
            <th>RESUMEN DE ASOCIACIONES</th>
        </tr>
    </thead>
    <tbody id="form-list-client-body">
        <?php
        include_once("../php/conexion.php");
        $query = "SELECT asocia.id_asociacion, demandante.nombre, perfil.descripcion FROM asocia INNER JOIN demandante INNER JOIN perfil ON asocia.dni = demandante.dni AND asocia.cod_perfil = perfil.cod_perfil";
        $sentencia= $bd -> prepare($query);
        $sentencia->execute();
        ?>
        <tr>
        <td><?php foreach($sentencia as $asocia) echo $asocia['id_asociacion'].' | Demandante: '.$asocia['nombre'].' | Perfil: '.$asocia['descripcion'].'<br/>' ?></td>
        </tr>
    </tbody>
</table></center>
<br><br><br>
