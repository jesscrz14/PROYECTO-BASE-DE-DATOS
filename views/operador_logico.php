...
<!-- OPERADOR LÓGICO AND -->
    ...
    <section class="inner-page">
      <div class="container">
              <h4>Tabla "Experiencia", "Demandante" y "Currículum".</h4>
              <p>Se hará una consulta donde acompletaremos la información del demandante, para conocer el id del currículum, su especialidad, universidad donde egresó, además del puesto en la anterior empresa donde laboró. </p>
              <p>Operador a implementar: AND</p>
        <form class="form-inline" method="POST" >
           <div class="form-group mx-sm-3">
             <label for="inputf1" class="sr-only">Ingresa el DNI del demandante</label>
               <input type="text" class="form-control" id="input1" placeholder="Ingresa el DNI del demandante" name="buscar" style="width: 20rem;" required >
            </div> 
            <div class="form-group mx-sm-3">   
            ...
            <a class="btn btn-primary" href="../report/reporte.php" role="button">Reporte General</a>   
        </form>
        <br><br>
    ...
    $buscar=$_POST['buscar'];
    $respuesta = $bd->prepare("SELECT demandante.dni, demandante.nombre, curriculum.id_cv, curriculum.especialidad, curriculum.universidad, experiencia.puesto, experiencia.nom_empresa FROM demandante INNER JOIN curriculum INNER JOIN experiencia WHERE demandante.dni='".$buscar."' AND demandante.dni = experiencia.dni AND demandante.id_cv = curriculum.id_cv");
      $respuesta->execute();
      $alquileres = [];
      foreach ($respuesta as $res) {
        echo "<tr>";
        echo '<td>' . $res['dni'] . '</td>';
        echo '<td>' . $res['id_cv'] . '</td>';
        echo '<td>' . $res['nombre'] . '</td>';;
        echo '<td>' . $res['especialidad'] . '</td>';
        echo '<td>' . $res['universidad'] . '</td>';
