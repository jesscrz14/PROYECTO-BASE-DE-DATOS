...
<body>
  ...
  <tbody>
    ...
    <?php include_once("../php/listar_demandante.php");?>
    <?php foreach ($demandante as $demandante) {?>

    <!-- SE MOSTRARÁ CADA UNO DE LOS REGISTROS" POR COLUMNAS-->
  
    <tr class="text-center">
      <td><?php echo $demandante->dni;?></td>
      <td><?php echo $demandante->nombre;?></td>
      <td><?php echo $demandante->direccion;?></td>
      <td><?php echo $demandante->telefono;?></td>
      <td><?php echo $demandante->cc_conducir;?></td>
      <td><?php echo $demandante->id_cv;?></td>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Nuevo Demandante</h5>
      ...
      <div class="modal-body">
        <!-- FORMULARIO -->
        <form action="../php/insertar_demandante.php" method="POST">
          ...
          <div class="form-group">
            <label>CARNET</label>
            <input type="text" name="cc_conducir" class="form-control">
          </div>
...
<div class="modal fade" id="editar" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Editar Demandante</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <!-- FORMULARIO -->
        <form action="../php/editar_demandante.php" method="POST">
          <input type="hidden" name="dni" id="update_dni">
          ...
          <div class="form-group">
            <label>CARNET</label>
            <input type="text" name="cc_conducir" id="cc_conducir" class="form-control">
          </div>

           <div class="form-group">
            <label>CV</label>
            <input type="text" name="id_cv" id="id_cv" class="form-control">
          </div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body"> <!-- AQUI SE REALIZARÁ LA BAJA -->
        <h4>¿Está seguro de eliminar al demandante?</h4>
        <p>Recuerda: no puedes eliminar al demadante si este se encuentra ligado a un registro de otra tabla. Deberás eliminar primero los otros registros.</p>
        <br>
        
        <form action="../php/eliminar_demandante.php" method="POST">
        <input type="hidden" name="dni" id="eliminar_id"> <!-- DE TIPO HIDDEN PARA OCULTAR LOS DATOS --> 
...
<!-- contador para registrar cuantos demandantes tenemos -->
<center><h4>Selección de tipo contador para saber la cantidad de demandantes</h4> 
<table class="table table-hover" style="width: 20rem;">
    <thead>
        <tr>
            <th>TOTAL</th>
        </tr>
    ...
    <tbody id="form-list-client-body">
        ...
        $query = "SELECT COUNT(dni) AS contador FROM demandante";
        $sentencia= $bd -> prepare($query);
        $sentencia->execute();
        ?>
        <tr>
            <td><?php echo $sentencia->fetch()['contador'] ?></td>
        </tr>
    </tbody>
</table></center>
...
<script>
  $('.editarbtn').on('click',function(){
    ...
    $('#update_dni').val(datos[0]);
    $('#nombre').val(datos[1]);
    $('#direccion').val(datos[2]);
    $('#telefono').val(datos[3]);
    $('#cc_conducir').val(datos[4]);
    $('#id_cv').val(datos[5]);
  });
</script>
